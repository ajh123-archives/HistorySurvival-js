...
import {triTable, edgeTable} from './variables'


class MarchingCubes {
	constructor(width, height, depth, chunk) {
		this.chunk = chunk;
	...
	makeShape(brushSize, color, point, multiplier, distanceFunction) {
		...
		for (let x = -brushSize - 2; x <= brushSize + 2; x++) {
			for (let y = -brushSize - 2; y <= brushSize + 2; y++) {
				for (let z = -brushSize - 2; z <= brushSize + 2; z++) {
					if (distanceFunction(point.clone(), new Vector3(point.x + x, point.y + y, point.z + z), brushSize) < 0) {
						mainScene.marchingcubes.set(Math.round(point.x + x), Math.round(point.y + y), Math.round(point.z + z), mainScene.marchingcubes.get(Math.round(point.x + x), Math.round(point.y + y), Math.round(point.z + z)) - distanceFunction(point.clone(), new Vector3(point.x + x, point.y + y, point.z + z), brushSize) * (multiplier));
					}
					if (distanceFunction(point.clone(), new Vector3(point.x + x, point.y + y, point.z + z), brushSize) < 1 && multiplier > 0) {
						const color = mainScene.marchingcubes.getColor(Math.round(point.x + x), Math.round(point.y + y), Math.round(point.z + z));
						const blendFactor = distanceFunction(point.clone(), new Vector3(point.x + x, point.y + y, point.z + z), brushSize) < 0 ? 1 : 0.5; //- (sphereDistance(point, new THREE.Vector3(point.x + x, point.y + y, point.z + z), brushSize) + brushSize) / (brushSize + 1);
						const finalColor = [brushColor[0] * blendFactor + color[0] * (1 - blendFactor), brushColor[1] * blendFactor + color[1] * (1 - blendFactor), brushColor[2] * blendFactor + color[2] * (1 - blendFactor)];
						mainScene.marchingcubes.setColor(Math.round(point.x + x), Math.round(point.y + y), Math.round(point.z + z), ...finalColor);
					}
				}
	...
	setMesh(geo, surfaceLevel) {
		...
		for (let x = 0; x < this.width; x ++) {
			for (let z = 0; z < this.depth; z ++) {
				for (let y = 0; y < this.height; y += 1) {
					...
					// Get each voxel.
					const ev0 = this.chunk.getVoxel(offsetX, offsetY, offsetZ);
					const ev1 = this.chunk.getVoxel(offsetX + 1, offsetY, offsetZ);
					const ev2 = this.chunk.getVoxel(offsetX + 1, offsetY, (offsetZ + 1));
					const ev3 = this.chunk.getVoxel(offsetX, offsetY, (offsetZ + 1));
					const ev4 = this.chunk.getVoxel(offsetX, offsetY + 1, offsetZ);
					const ev5 = this.chunk.getVoxel(offsetX + 1, offsetY + 1, offsetZ);
					const ev6 = this.chunk.getVoxel(offsetX + 1, offsetY + 1, (offsetZ + 1));
					const ev7 = this.chunk.getVoxel(offsetX, offsetY + 1, (offsetZ + 1));

					// Get the voxel's amount
					const e0 = ((ev0 !== undefined)? ev0.amount : -1);
					const e1 = ((ev1 !== undefined)? ev1.amount : -1);
					const e2 = ((ev2 !== undefined)? ev2.amount : -1);
					const e3 = ((ev3 !== undefined)? ev3.amount : -1);
					const e4 = ((ev4 !== undefined)? ev4.amount : -1);
					const e5 = ((ev5 !== undefined)? ev5.amount : -1);
					const e6 = ((ev6 !== undefined)? ev6.amount : -1);
					const e7 = ((ev7 !== undefined)? ev7.amount : -1);
					// Calculate the cubIndex to use 
					let cubeindex = 0;
					...
					console.log(x, y, z, edge)
					if (edge !== 0) {
						// Get the voxels we need to calculate each normal
						// const nv0 = this.chunk.getVoxel(offsetX + 1, offsetY, offsetZ);
						// const nv1 = this.chunk.getVoxel(offsetX - 1, offsetY, offsetZ);
						// const nv2 = this.chunk.getVoxel(offsetX, offsetY + 1, offsetZ);
						// const nv3 = this.chunk.getVoxel(offsetX, offsetY - 1, offsetZ);
						// const nv4 = this.chunk.getVoxel(offsetX, offsetY, offsetZ + 1);
						// const nv5 = this.chunk.getVoxel(offsetX, offsetY, offsetZ - 1);
						// Get the voxel's amount
						// const n0 = ((nv0 !== undefined)? nv0.amount : -1);
						// const n1 = ((nv1 !== undefined)? nv1.amount : -1);
						// const n2 = ((nv2 !== undefined)? nv2.amount : -1);
						// const n3 = ((nv3 !== undefined)? nv3.amount : -1);
						// const n4 = ((nv4 !== undefined)? nv4.amount : -1);
						// const n5 = ((nv5 !== undefined)? nv5.amount : -1);
		...
		geo.setIndex(new BufferAttribute(indices.slice(0, idxCounter), 1));
		// console.log(this.voxelBuffer)
		geo.setAttribute('position', new BufferAttribute(vertices.slice(0, vIdx), 3));
		// geo.setAttribute('normal', new BufferAttribute(normals.slice(0, vIdx), 3));
		geo.setAttribute('color', new BufferAttribute(colors.slice(0, vIdx), 3));
